...
class Chat:
    ...
    def assistant_history(self, text, conv):
        conv.append_message(conv.roles[1], text)

    def answer_prepare(self, conv, img_list, max_new_tokens=300, do_sample=True, num_beams=5, min_length=1, top_p=0.9,
                       repetition_penalty=1.05, length_penalty=1, temperature=1.0, max_length=2000, output_scores=False, stopping_criteria=None):
        conv.append_message(conv.roles[1], None)
        prompt = conv.get_prompt()
        embs = self.model.get_context_emb(prompt, img_list, self.device)
        ...
        generation_kwargs = dict(
            ...
            stopping_criteria=self.stopping_criteria if stopping_criteria is None else stopping_criteria,
            num_beams=num_beams,
            do_sample=do_sample,
