...
class AdvancedEval(BaseEval):
    EVAL_DESCRIPTION = "The chat bot is presented with an image and a difficult question which need it think for a while. We will ask the chat bot to answer this question."
                        
    task_list = ["d-advanced-spatial","d-advanced-temporal","d-advanced-compare","d-advanced-daily","d-advanced-traffic","d-advanced-causality","d-advanced-math","d-advanced-code","d-advanced-translate"]
    ...
    def __init__(self, task_id: str) -> None:
        ...
        print("Test for Advanced Cognitive Inference is Preparing.")
        print("task_id",task_id)
        self.image_dir = "/data/zhangyichi/Trustworthy-MLLM/data/truthfulness/advanced_images/"
        ...
        self.labels = []
        self.images = []
        query_file = "/data/zhangyichi/Trustworthy-MLLM/data/truthfulness/query_advanced/query_d_{}.json".format(task_id.split('-')[-1])
    ...
    def eval(self, test_model: BaseChat, log_response:Optional[str]=None, **kwargs):
        ...
        for statement in tqdm(self.statements):
            id += 1
            test_prompt = AdvancedEval.PROMPT_TEMPLATE.format(Statement=statement)
    ...
    def eval_response(self, log_file:Union[str|List]):
        ...
        YES_CANDIDATES = ["yes", "Yes", "visible", "[Yes]", "[yes]"]
        NO_CANDIDATES = ["No", "no", "Wrong", "wrong", "not", "there is no", "[No]", "[no]"]
