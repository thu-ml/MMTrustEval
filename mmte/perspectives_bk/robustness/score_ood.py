...
# ood
input_dir='/data/zhangyichi/Trustworthy-MLLM/output/robustness/description_ood'
output_dir='/data/zhangyichi/Trustworthy-MLLM/output/robustness/score_ood'
os.makedirs(output_dir, exist_ok=True)
log_dir='/data/zhangyichi/Trustworthy-MLLM/output/robustness/score_ood/alog.txt'
        ...
        for category in categories:
            try:
                ...
                for image_info in input_json_list:
                    label=image_info['gt_label']
                    response=image_info['response']
                    label_str=', '.join(label)

                    # score
                    prompt=f'You now play the role of a rater, evaluating the chatbot\'s description to a scene in {category} style. The description should contain {label_str}. The chatbot\'s description is \'{response}\'. Please give a score from 0, 25, 50, 75, 100. Just give the score.'
                ...
                log_file.write(f'Average score of model {model_name} on coco {category} is: {total_score/total_num}.\n')
