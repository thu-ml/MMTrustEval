    ...
    def eval(self, **generation_kwargs):
        ...
        with open(self.log_file, 'w') as log_output:
            for category, image_dir in self.categories.items():
                ...
                with open(os.path.join(self.root_dir, f'caption.json'), 'r') as f:
                    coco_caption_dict=json.load(f)
                with open(os.path.join(self.root_dir, 'label_exist.json'), 'r') as f:
                    coco_label_exist_dict=json.load(f)
                with open(os.path.join(self.root_dir, 'label_nonexist.json'), 'r') as f:
                    coco_label_nonexist_dict=json.load(f)

                # evaluation
                # correct_num=0
                # total_num=0
                response_list=[]
                for image_id, image_info in coco_label_exist_dict.items():
                    try:
                        image_name=image_info['image_name']
                        label_exist=image_info['label']
                        label_nonexist=coco_label_nonexist_dict[image_id]['label']
                        caption=coco_caption_dict[image_id]['label']
                    ...
                    response_list.append({'image_id': image_id, 'image_name': image_name, 'question': text, 'gt_caption': caption, 'gt_label': label_exist, 'target_label': label_nonexist, 'response': response_txt})
...
parser.add_argument('--root_dir', type=str, default='/data/zhangyichi/Trustworthy-MLLM/data/robustness/at', help='Path to the input images.')
parser.add_argument('--output_dir', type=str, default='/data/zhangyichi/Trustworthy-MLLM/output/robustness/description_at', help='Output dir.')
