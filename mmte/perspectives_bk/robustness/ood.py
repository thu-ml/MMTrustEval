...
class OODEval():
    def __init__(self, args, model, model_name, root_dir='', out_dir='', log_file='') -> None:
        ...
        self.label_dirs = ['coco-labels-sample-200', 'coco-labels-sample-200-nonexist']
    ...
    def eval(self, **generation_kwargs):
        ...
        with open(self.log_file, 'w') as log_output:
            for label_dir in self.label_dirs:
                for coco_type, image_dir in self.categories.items():
                    with open(os.path.join(self.root_dir, label_dir, f'{coco_type}.json'), 'r') as f:
                    ...
                    log_output.write(f'Correct ratio on coco type {coco_type} of {label_dir} samples is {correct_num/total_num}.\n')
                    with open(os.path.join(self.out_dir, f'{coco_type}_{label_dir}_response.json'), 'w') as out_file:
...
parser = argparse.ArgumentParser(description='OOD robustness evaluation.')
...
parser.add_argument('--output_dir', type=str, default='/data/zhangyichi/Trustworthy-MLLM/output/robustness/ood', help='Output dir.')
