...
class AdvGlueEval():
    def __init__(self, args, model, model_name, root_dir='', out_dir='', log_file='') -> None:
        ...
        self.categories = ['mnli-mm', 'qnli', 'qqp', 'rte', 'sst2']
    ...
    def eval(self, **generation_kwargs):
        ...
        with open(self.log_file, 'w') as log_output:
            for category in self.categories:
                ...
                # input data
                with open(os.path.join(self.root_dir, 'dev_ann.json'), 'r') as f:
                ...
                for text_info in test_data_dict[category]:
                    ...
                    if category == "sst2":
                        tmp_label = f'original_{text_keys[0]}' if f'original_{text_keys[0]}' in text_info else text_keys[0]
                        text=text_prompt.format(text_info[tmp_label])
                    else:
                        tmp_label1 = f'original_{text_keys[0]}' if f'original_{text_keys[0]}' in text_info else text_keys[0]
                        tmp_label2 = f'original_{text_keys[1]}' if f'original_{text_keys[1]}' in text_info else text_keys[1]
                        text=text_prompt.format(text_info[tmp_label1], text_info[tmp_label2])
...
parser.add_argument('--root_dir', type=str, default='/data/zhangyichi/Trustworthy-MLLM/data/robustness/advglue', help='Path to the input images.')
parser.add_argument('--output_dir', type=str, default='/data/zhangyichi/Trustworthy-MLLM/output/robustness/description_advglue_original', help='Output dir.')
